...
def run_build(
    ...
    *,
    ...
    failed = sum(1 for result in results if not result["compile_success"])
    ...
    for record in records:
        ...
        for variant in variants:
            ...
            for opt_level in opt_levels:
                ...
                compile_flags = [f"-{opt}", *VARIANT_DEFINES[variant]]
...
def append_support_todo(stderr: str) -> str:
    todo = (
        "\nTODO: MVP currently compiles a single testcase source file. "
        "If Juliet common support files, include paths, or extra source files are required, "
        "extend src/juliet/builder.py to add testcase support sources and include directories."
    )
